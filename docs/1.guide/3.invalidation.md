...
Use the standalone `invalidateCache()` when you don't hold a reference to the cached function — for example from a webhook or a different module. Pass the same key options (`name`, `getKey`, and `group` / `base` if you customized them) **and the same `storage`**:
...
> [!IMPORTANT]
> The standalone helpers rebuild the cache key from the options you pass — they can't see the original function. They must match the `name`, `getKey`, `group`, and `base` you cached with, or they'll target a different key and no-op. The same goes for `storage`: it is per cached function, so options without one resolve a fresh, empty store and purge nothing. Passing the same options _object_ is the safest form — the resolved storage is memoized onto it.
...
The standalone `resolveCacheKeys()` does the same without a function reference (again, pass matching key options), and you read the entries from the storage the function caches into:
...
> [!IMPORTANT]
> `invalidateCache()` and `expireCache()` act on `options.storage`, and storage is **per cached function** — there is no global store to fall back on, so they **throw** if `storage` is unset rather than purging an empty store while the stale entry keeps being served. Pass the **same options object** you cached with (its resolved storage is memoized onto it), or at least the same `storage`. When the cached function itself is at hand, prefer its own `.invalidate()` / `.expire()`. Note a mismatched `name`/`getKey` still purges nothing silently — only the missing backend is caught.
