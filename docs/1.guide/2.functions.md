...
**Leaving it unset means the stale window is unbounded**: the last successful value is served for as long as the entry exists, and no storage TTL is armed — the entry is retained until the storage backend evicts it (the [ISR](/guide/isr) shape; `createMemoryStorage` bounds entry _count_, not bytes).
