...
A non-storable **status** also suppresses `Cache-Control` synthesis: ocache never advertises a lifetime for a response it will not store itself. A `500` is returned to the caller with no synthesized `Cache-Control` at all, so a shared cache in front of your handler doesn't pin the error for `maxAge + staleMaxAge` while ocache re-runs the handler on every request. (A `Cache-Control` the handler set itself is always preserved, whatever the status.)
