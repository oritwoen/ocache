...
The default storage — a fresh one per cached function/handler — is created with `createMemoryStorage`. It keeps at most **10 000** entries by default, evicting the least-recently-used entries once the ceiling is exceeded (LRU). Reading or writing a key marks it as most-recently-used.
...
const storage = createMemoryStorage({ maxSize: 50_000 });

// Opt out of the ceiling entirely.
const unbounded = createMemoryStorage({ maxSize: Number.POSITIVE_INFINITY });
