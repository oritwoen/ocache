...
> [!CAUTION]
> **Credential headers such as `Authorization` are not part of the cache key.** The auto-generated key is the URL path plus whatever you opted into via `varies`, [`allowQuery`](/guide/query-params), and [`allowCookies`](/guide/cookies) — nothing else. A bearer token, session token, or API key does **not** create a separate entry, so if your handler's response depends on one, the first caller's response is stored and replayed to everyone else requesting the same URL. Unlike cookies, these headers get no automatic protection: cookies are attached by the browser to every request, so ocache strips them by default, but an `Authorization` header is only ever sent deliberately, and stripping it would break the routes that legitimately need it.
>
> For a route whose response is scoped to the caller, either skip the cache for those requests:
>
> ```ts
...
> ```
>
> or, if the response really is shareable between everyone presenting the same credential, put it in the key with `varies: ["authorization"]` — keeping in mind that this stores one entry per distinct token value.
