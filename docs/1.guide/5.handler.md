...
- **Cache keys** — auto-generated from the request URL path (plus any query params and headers you opt into varying, see below) and the request method.
...
- **`Vary`** — advertises which request headers the response varies on (from `varies`, plus `Cookie` when `allowCookies` is set).
...
> [!TIP]
> **Varying headers stay visible to the handler.** Their value is part of the cache key, so rendering from them is safe — each distinct value gets its own entry. (Cookies and credential headers are handled separately — see [Cookies](/guide/cookies) and the note below.)
...
Only cacheable requests are affected: `POST` and friends bypass the cache and reach the handler with their credentials intact.
